...
ROOT = OUT / NAME
RAW_DIR = ROOT / "raw/per_product"
FLAT_DIR = ROOT / "flat/per_product"
META = ROOT / "metadata"
for p in (RAW_DIR, FLAT_DIR, META):
...
with combined_raw.open("w", newline="", encoding="utf-8-sig") as arf, combined_flat.open("w", newline="", encoding="utf-8-sig") as aff:
    ...
    for product in PRODUCTS:
        ...
        raw_path, flat_path = RAW_DIR / f"{product}.csv", FLAT_DIR / f"{product}.csv"
        count, first, last = 0, None, None
        statuses, validations, sources = Counter(), Counter(), Counter()
        with raw_path.open("w", newline="", encoding="utf-8-sig") as rf, flat_path.open("w", newline="", encoding="utf-8-sig") as ff:
            rw, fw = csv.DictWriter(rf, RAW), csv.DictWriter(ff, FLAT)
            ...
            for src in source_files:
                ...
                with src.open(newline="", encoding="utf-8-sig") as handle:
                    ...
                    for row in reader:
                        ...
                        rw.writerow(raw_row); fw.writerow(flat); arw.writerow(raw_row); afw.writerow(flat)
...
lines += ["", "## Tệp chính", "", "- `raw/vietlott_draws_5y_raw.csv`: giữ nguyên JSON và trường nguồn.",
          ...
          "- Các thư mục `per_product`: tách riêng từng sản phẩm.",
